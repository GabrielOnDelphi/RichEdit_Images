...
type
  TEditorMainForm = class(TForm)
    App                 : TAppEvents;
    BackgroundMenu      : TPopupMenu;
    BoldBtn             : TSpeedButton;
    BulletsBtn          : TSpeedButton;
    ...
    SubscriptBtn        : TSpeedButton;
    SuperscriptBtn      : TSpeedButton;
    ToolbarImages       : TImageList;
    UnderlineBtn        : TSpeedButton;
    UnderlineStyle: TPopupMenu;
...
procedure TEditorMainForm.BoldBtnClick(Sender: TObject);
begin
  if FUpdating then
    Exit;
...
begin
  if FUpdating then
    Exit;
  if FontSize.Value > 0 then
    CurrText.Size := FontSize.Value;
end;
  ...
  begin
    ...
    begin
      Item := NewItem(UnderlineTypes[TUnderlineType(I)], scNone,
        False, True, UnderlineBtnClick, 0, '');
      Item.RadioItem := True;
      Item.Tag := I;
      Add(Item);
    end;
...
procedure TEditorMainForm.ItalicBtnClick(Sender: TObject);
begin
  if FUpdating then
    Exit;
...
procedure TEditorMainForm.UnderlineBtnClick(Sender: TObject);
begin
  if FUpdating then
    Exit;
